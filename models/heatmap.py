...
version = '1.02'
# apihost = 'http://geodev.lib.purdue.edu/heat/'
#apihost = 'http://localhost:8888/heatmap/'
apihost = 'http://geo.lib.purdue.edu/heatmapr/'
...
def setup_options():
  ...
  optparser.add_option('-q', '--postgresql', metavar='STR')	
  optparser.add_option('-g', '--gpx', metavar='FILE')
  optparser.add_option('-j', '--geojson', metavar='FILE', help='File containing one space-separated coordinate pair per line')
...
def main():
  ...
  if not ((options.points or options.gpx or options.geojson or options.postgresql or options.load) and (options.output or options.save)):
    sys.stderr.write("You must specify one input (-g -p -L) and at least one output (-o or -S).\n")
    sys.exit(1)

  if (options.gpx or options.points or options.geojson or options.postgresql) and not ((options.width or options.height or options.scale or options.background_image)
  ...
  if options.output:
  #need to do some hackwork to resolve system path vs. public output discrepancy
    publicout = options.output
    options.output = "/var/www/html/heatmapr/"+options.output
    colormap = ColorMap()
    if options.gradient:
      from PIL import Image
  ...
  if options.load:
    ...
    if options.gpx:
      ...
      shapes = []
      gj_stream=urllib.urlopen(options.geojson).read()
      pcl_source=geojson.loads(gj_stream);
      feature_list=pcl_source['features']
      for item in feature_list[1:200]:
	 coordlist=item['geometry']['coordinates']
        ...
        printtime('reading points from postgresql database')
        shapes = []
	try:
		pgconfigf = open("/var/pugeo/pgconfig.txt","r")
		pgarray=[]		
		for line in pgconfigf:
			pgarray.append( line )		
		conn = psycopg2.connect("dbname="+pgarray[1]+" user="+pgarray[2]+" host="+pgarray[0]+" password="+pgarray[3])
	except:
	    print "I am unable to connect to the database"
	cur = conn.cursor()
	cur.execute("""select st_astext(the_geom) from """+options.postgresql+""" limit 100""" )
	rows = cur.fetchall()
	for row in rows:
		if (row[0]!=None):
#			print row[0][6:-1];
			(lon,lat) = [float(x) for x in row[0][6:-1].split()]
        		shapes.append(Point((lat,lon)))
      	if options.verbose:
        	printtime('read %d points' % len(shapes))
